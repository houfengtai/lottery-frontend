  ...
  methods: {
    _initData () {
      axios.get('/api/admin/activites').then(res => {
        ...
        if (data.length > 0) {
          ...
          this.item.activityName = data[0].activityName
          this._findTrophys()
        }
        this.activites = data
      })
    ...
    _findTrophys () {
      axios.get(`/api/admin/trophes?activityId=${this.item.activityId}`).then(res => {
        const data = res.data.data
        if (data.length > 0) {
          this.item.trophyId = data[0].id
          this.item.trophyName = data[0].trophyName
        }
        this.trophes = data
      })
    },
    _save () {
      ...
      axios.post('/api/admin/setting/user', param).then(res => {
